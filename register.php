...
$username_err = $password_err = $confirm_password_err = $lname_err = $email_err = $fname_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	//validate at least 1 hobby is chosen:
	if(isset($_POST['formHobbies'])) {
		$aHobbies = $_POST['formHobbies'];
		$nHobbies = count($aHobbies);
	}
  else if(!isset($aHobbies)) {
	$email_err = "Please select at least one hobby.";
	}
  
    // Check input errors before inserting in database
    if(empty($username_err) && empty($password_err) && 
	empty($email_err) && empty($fname_err) && 
	empty($lname_err) && empty($confirm_password_err))
	{
        ...
        if($stmt = mysqli_prepare($link, $sql)){
			...
			$param_email = $email;
            //add a hobbies list to pick from!
            // Attempt to execute the prepared statement
            if(mysqli_stmt_execute($stmt)){
...
<head>
    ...
    <style>
        body{ font: 14px sans-serif; background-color: #f7fef6; }
...
<body>
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
			...
			<div class="form-group">
			<label for='formHobbies[]'>CTRL-Select Your Hobbies:</label><br>
